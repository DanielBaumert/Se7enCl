...
namespace Se7enCl
{
    ...
    public readonly unsafe struct Context : IRefCountedHandle
    {
        ...
        public readonly uint GetReferenceCount()
           => Utils.GetTInfo<ContextInfo, uint>(Handle, ContextInfo.ReferenceCount, OpenCl.GetContextInfo);
        public readonly string GetProperties()
            => Utils.GetTInfo<ContextInfo, byte>(Handle, ContextInfo.Properties, OpenCl.GetContextInfo, out _).ToStrg();
        public readonly Device[] GetDevices()
            => Utils.GetTInfo<ContextInfo, Device>(Handle, ContextInfo.Devices, OpenCl.GetContextInfo, out _);

        #region IRefCountedHandle Members

        public ErrorCode Retain()
            => OpenCl.RetainContext(Handle);

        public ErrorCode Release()
            => OpenCl.ReleaseContext(Handle);
