...
namespace Se7enCl
{
    ...
    public unsafe readonly struct SvmPointer : IDisposable
    {
        ...
        internal ErrorCode Lock(CommandQueue queue, IntPtr length)
            => OpenCl.EnqueueSVMMap(queue, true.ToInt(), MapFlags.Read | MapFlags.Write, this, length, 0, null, out _);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal ErrorCode Unlock(CommandQueue queue)
            => OpenCl.EnqueueSVMUnmap(queue, this, 0, null, out _);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void Dispose() => OpenCl.SVMFree(_ctx, _handle);
    ...
    public readonly unsafe struct SVMCoarseGrainBuffer : IDisposable
    {
        ...
        public SVMCoarseGrainBuffer(Context ctx, CommandQueue commandQueue, IntPtr length) {
            _queue = commandQueue;
            _pointer = new SvmPointer(ctx, OpenCl.SVMAlloc(ctx, SVMMemFlags.ReadWrite | SVMMemFlags.FineGrainBuffer, length));
            Length = length;
        }
